...
public class Config {
    ...
    public Config(DatagramSocket commUdpSocket, InetSocketAddress peerAddr){
        this.commUdpSocket = commUdpSocket;
        this.peerAddr = peerAddr;
    }
    public void sendQXAFSConfig(List<String> configs, int channel){
...
class SendConfig implements Runnable{
    ...
    private int channel;
    public volatile boolean isRecv = false;
...
class RecvConfig implements Runnable{
    ...
    private int getSeq(byte[] config) {
        //回传指令的32-47位代表指令序号
        //对应byte数组的第4、5
        return (config[2] << 8 | config[3]) & 0xffff;
    }
}